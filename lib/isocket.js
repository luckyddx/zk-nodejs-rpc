...
ISocket.prototype.send = function(event, params, callback) {
    ...
    var message = { _dver: '2.4.9',
        _interface: 'com.raycloud.dubbo.request.KmzsNodeTest',
        _version: '0.0.1',
        _group: undefined,
        _timeout: 6000,
        _method: 'test',
        _args: [ { '$class': 'java.lang.String', '$': 'zzzzxiao' } ] }
    var str = {
        $class: 'java.lang.String',
        $: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxx'
    };

    //var buff = new Encode(encoder.write(str));
    var buff = new Encode(message);
    console.log(buff);
...
ISocket.prototype.data = function(event, listener) {
    console.log([ "data" ].concat(event),'进程监听处理事件');
...
ISocket.prototype._onData = function(buf) {
    ...
    console.log(buf,'->->->->->->->->->->->->.');
    console.log(buf[2],'==============================');
    console.log('mmmmm------', buf.slice(4, 12).toString(),'-mmmmmmmmmm');
    var mid = buf.slice(4, 12).toString();
    mid = 1;
    console.log('===========', buf.slice(16, buf.length - 1).toString(),'--获取到的请求');
    if(this.$buff) {
        buf = Buffer.concat([ this.$buff, buf ], this.$buff.length + buf.length);
    }

    event = [mid, 'call', 'com.raycloud.dubbo.request.KmzsNodeTest.test', 'test'];
    data = 'test00000';
    console.log('触发事件');
    console.log([ "data" ].concat(event),'-----触发');
