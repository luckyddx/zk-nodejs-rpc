 ...
 * @constructor
 */
var Zookeeper = function(connectString, options, root, prefix, version) {
    ...
    this.root = root || "/dubbo";
    this.prefix = prefix || "dubbo_";
    ...
    if(!this.root.length) this.root = "/dubbo";
    if(!this.prefix.length) this.prefix = "dubbo";
...
Zookeeper.prototype._register = function(callback) {
    ...
    var path = this.root + '/'+encodeURIComponent(this.prefix);
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            if(null === self.path) {
                client.create(path, data, CREATE_MODE.EPHEMERAL, callback);
            } else {
